<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:161b22,100:4facfe&height=220&section=header&text=Prakash%20Kumar%20Sarangi&fontColor=e8e8f0&fontSize=42&fontAlignY=35&desc=Software%20Engineer%20%7C%20Systems%20%7C%20AI/ML%20%7C%20Full-Stack&descSize=16&descColor=9ca3af&descAlignY=55&animation=fadeIn"/>

<div align="center">

<picture>
  <img alt="Terminal" src="terminal.svg" width="100%" />
</picture>

<br><br>

<a href="https://www.linkedin.com/in/prakash1209/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>&nbsp;
<a href="mailto:prakash_m251250cs@nitc.ac.in">
  <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>&nbsp;
<a href="https://github.com/prakash-nitc">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>&nbsp;
<img src="https://komarev.com/ghpvc/?username=prakash-nitc&style=for-the-badge&color=4facfe&label=PROFILE+VIEWS" />

</div>

---

## <img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="30"> &nbsp;About Me

I'm a software engineer who loves **building systems from the ground up** — from low-level C socket programming to full-stack web platforms to neural networks implemented with nothing but NumPy.

- 🎓 **M.Tech** in Computer Science at **NIT Calicut**, researching multimodal anomaly detection with Vision-Language Models
- 🔧 I enjoy working at **every layer of the stack** — TCP sockets in C, Spring Boot microservices, React frontends, and ML pipelines
- 🧠 I implement ML algorithms **from scratch** to truly understand the math, not just the APIs
- 🚀 Currently building **AI-powered developer tools** and **distributed systems**

---

## 🛠️ &nbsp;Tech Stack

<table>
<tr>
<td valign="top" width="33%">

### Languages
<p>
<img src="https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white"/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/C/C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white"/>
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white"/>
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black"/>
<img src="https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white"/>
<img src="https://img.shields.io/badge/HTML/CSS-E34F26?style=flat-square&logo=html5&logoColor=white"/>
</p>

</td>
<td valign="top" width="33%">

### Frameworks & Tools
<p>
<img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white"/>
<img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black"/>
<img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white"/>
<img src="https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white"/>
<img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white"/>
<img src="https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white"/>
<img src="https://img.shields.io/badge/HTMX-3366CC?style=flat-square&logo=htmx&logoColor=white"/>
</p>

</td>
<td valign="top" width="33%">

### Infrastructure
<p>
<img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white"/>
<img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white"/>
<img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white"/>
<img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white"/>
<img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black"/>
<img src="https://img.shields.io/badge/WebSocket-010101?style=flat-square&logo=socketdotio&logoColor=white"/>
<img src="https://img.shields.io/badge/POSIX-000000?style=flat-square&logo=gnu&logoColor=white"/>
</p>

</td>
</tr>
</table>

---

## 🏗️ &nbsp;Featured Projects

<table>
<tr>
<td width="50%">

### 🤖 [InterviewPilot](https://github.com/prakash-nitc/InterviewPilot)
**AI-Powered Mock Interview Platform**

Practice technical interviews with AI that generates questions, evaluates responses in real-time, and provides detailed scoring with feedback.

`Java` `Spring Boot` `HTMX` `Gemini API` `Thymeleaf`

</td>
<td width="50%">

### ⚡ [Distributed Task Scheduler](https://github.com/prakash-nitc/Distributed-Task-Scheduler)
**Master-Worker Architecture in C**

Low-level distributed system using TCP sockets, `fork/exec`, `dup2`, `pipe`, `select()` — no frameworks, pure POSIX APIs with load balancing.

`C` `TCP Sockets` `POSIX` `IPC` `fork/exec`

</td>
</tr>
<tr>
<td width="50%">

### 🧠 [Multi-Agent AI Simulation](https://github.com/prakash-nitc/Multi-Agent-Generative-AI-Simulation)
**Stanford Generative Agents Architecture**

8 autonomous AI agents with FAISS memory streams, reflection, planning, and real-time 3D visualization — inspired by Stanford's research paper.

`Python` `LangChain` `FAISS` `Three.js` `Flask`

</td>
<td width="50%">

### 🔤 [Neural Net Letter Classifier](https://github.com/prakash-nitc/neural-net-letter-classifier)
**From-Scratch Neural Network in NumPy**

Complete feedforward NN with backpropagation, Xavier init, architecture search, and a Flask web app — zero ML frameworks.

`Python` `NumPy` `Flask` `Matplotlib`

</td>
</tr>
<tr>
<td width="50%">

### 🚗 [RideConnect](https://github.com/prakash-nitc/rideconnect-app)
**Campus Carpooling Web App**

Production-ready platform with secure auth, ride search, fare splitting, and responsive UI designed for college campuses.

`TypeScript` `Next.js` `MongoDB` `NextAuth`

</td>
<td width="50%">

### 💼 [WAMS Backend](https://github.com/prakash-nitc/wams-backend)
**Workflow & Approval Management System**

Enterprise-grade backend for managing multi-level approval workflows with role-based access control.

`Java` `Spring Boot` `PostgreSQL` `JWT`

</td>
</tr>
</table>

<details>
<summary><b>🔬 More Projects</b></summary>
<br>

| Project | Description | Tech |
|---------|-------------|------|
| [🔍 Multimodal Anomaly Detection](https://github.com/prakash-nitc/multimodal-anomaly-detection) | M.Tech research — anomaly detection using Vision-Language Models on MVTec AD | Python, CLIP, PyTorch |
| [⚔️ CodeArena](https://github.com/prakash-nitc/CodeArena) | Real-time coding interview platform with WebSocket and JWT auth | Java, Spring Boot, WebSocket |
| [📚 Study Focus](https://github.com/prakash-nitc/Study_Focus) | Chrome extension to block distractions and track study sessions | JavaScript, Chrome APIs |
| [🌐 Portfolio Website](https://github.com/prakash-nitc/Portfolio_website) | Cosmic-themed portfolio with constellation animations and glassmorphism | HTML, CSS, JS |
| [💻 LeetCode Solutions](https://github.com/prakash-nitc/Leetcode_Solutions) | Curated problem solutions with clean code | Java |

</details>

---

## 📊 &nbsp;GitHub Analytics

<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=prakash-nitc&show_icons=true&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=4facfe&icon_color=a855f7&text_color=e8e8f0&ring_color=4facfe" />
  &nbsp;&nbsp;
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=prakash-nitc&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=4facfe&text_color=e8e8f0&langs_count=8" />
</div>

<br>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=prakash-nitc&theme=github-dark-blue&hide_border=true&background=0d1117&stroke=4facfe&ring=4facfe&fire=a855f7&currStreakLabel=4facfe&sideLabels=4facfe&dates=9ca3af" />
</div>

<br>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=prakash-nitc&bg_color=0d1117&color=4facfe&line=a855f7&point=e8e8f0&area=true&area_color=4facfe&hide_border=true" />
</div>

---

<div align="center">

### 💬 &nbsp;Let's Connect

*I'm always open to discussing software architecture, distributed systems, AI/ML, or exciting collaboration opportunities.*

**If my work interests you, let's talk!**

<a href="https://www.linkedin.com/in/prakash1209/">
  <img src="https://img.shields.io/badge/-Let's_connect_on_LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:161b22,100:4facfe&height=120&section=footer"/>
